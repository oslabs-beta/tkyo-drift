...
def get_cluster_centroids(num_vectors,dims):
    ...
    # trainingData = np.random.rand(50000, 700).astype('float16')

    trainingData = np.fromfile('data/concept.output.training.bin', dtype=np.float16)

    float16_size = np.dtype(np.float16).itemsize  # 2 bytes per float16

    expected_size = num_vectors * dims * float16_size
    actual_size = os.path.getsize('data/concept.input.training.bin')

    print(f"Expected size: {expected_size} bytes")
    print(f"Actual size:   {actual_size} bytes")
    print(f"Extra bytes:   {actual_size - expected_size} bytes (metadata?)")    
    ...
    # query = np.random.rand(dims).astype('float16')
