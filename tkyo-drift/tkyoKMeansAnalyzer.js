...
const readVectorsFromBin = async () => {
  ...
  const stream = fs.createReadStream(
    path.join(
      '/home/msymeono/Codesmith/projects/tkyo-drift',
      'semantic_input.training.bin'
    ),
    {
      highWaterMark: 1536,
    }
  ...
  for await (const chunk of stream) {
    ...
    const floatArray = new Float32Array(chunk.buffer, chunk.byteOffset, 384);
  ...
  const calculateInertia = (vectors, result) => {
    ...
    for (let i = 0; i < vectors.length; i++) {
      ...
      const euclideanDistance = Math.sqrt(
        vectors[i].reduce(
          (sum, val, idx) => sum + (val - centroid[idx]) ** 2,
          0
        )
      );
